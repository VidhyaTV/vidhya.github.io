...
void Process::ReceiveMsg(){
	...
	if (!mail_box.empty() && begin(mail_box)->arrival_time <= PhysicalTime::GetAbsoluteTime()) {
		...
		while (!mail_box.empty() && begin(mail_box)->arrival_time <= PhysicalTime::GetAbsoluteTime()) 
		{
			...
			/************code for predicate printing***********/
			ofstream wcp_predicate("wcp_predicate_a"+std::to_string(alpha)+"_e"+std::to_string(epsilon)+"_l"+std::to_string(localPredRate)+"_d"+std::to_string(delta)+"_v"+std::to_string(glob_true_interval)+"_run"+std::to_string(run)+".smt2",ios::out | ios::app);
			//ofstream wcp_predicate("wcp_predicate.smt2",ios::out | ios::app);
